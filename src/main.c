...
static char DumpFile(const char *filename, const char *outfile, Stream stream, Format format, Values values, Time time, Options options, float amplify, unsigned long iStart, unsigned long iLength, unsigned long iStep, int blockStart, int blockCount, int verbose)
{
    ...
    {
    	Rprintf("ERROR: No filename given. Necessary for SQLite.\n");
    	return -1;
    }
#endif
    ...
    if (verbose > 0) {
    	Rprintf("\rReading %d sectors (offset %d, file %d)...\n", numSectors, blockStart, lengthSectors);
    }
    for (n = 0; n < numSectors; n++)
    {
        unsigned long offset;
        //Rprintf("\rSECTOR %5d/%5d (%3d%%): ...\b\b\b", n, lengthSectors, 100 * n / lengthSectors);
        offset = (n + blockStart) * SECTOR_SIZE;
        fseek(fp, offset, SEEK_SET);

        if (sizeof(DataPacket) != SECTOR_SIZE)
        {
            Rprintf("SEVERE WARNING: DataPacket size not equal to a sector size!\n");
        }

        if (fread(buffer, 1, SECTOR_SIZE, fp) != SECTOR_SIZE)
        {
            Rprintf("ERROR: Problem reading sector.\n");
        }
        else
        {
				...
				else
				{
					...
					else if (format == FORMAT_RAW || format == FORMAT_WAV)
					{
						...
						else
						{
                            ...
                            if (dataPacket->sessionId != deviceSessionId) { Rprintf("!"); } else { Rprintf("*"); }
							...
							{
								Rprintf("[ERROR: RAW/WAV output does not support changes to the number of axes]");
							}
							...
							if (sequence >= iStart && sequence - iStart < iLength && ((sequence - iStart) % iStep) == 0)
							{
                                ...
                                if (values == VALUES_INT && format == FORMAT_SQLITE)
                                {
                                    if (sqlite3_bind_double(stmt, 1, (double)(t - 3600.0))) {
                                        Rprintf("ERROR: sqlite bind double (time).\n");
                                    }
                                    if (sqlite3_bind_int(stmt, 2, x)) {
                                        Rprintf("ERROR: sqlite bind int (x).\n");
                                    }
                                    if (sqlite3_bind_int(stmt, 3, y)) {
                                        Rprintf("ERROR: sqlite bind int (y).\n");
                                    }
                                    if (sqlite3_bind_int(stmt, 4, z)) {
                                        Rprintf("ERROR: sqlite bind int (z).\n");
                                    }
                            		...
                            		if (options & OPTIONS_LIGHT)
                            		{
                            			...
                            			{
											Rprintf("ERROR: sqlite bind int (light).\n");
										}
                            			j++;
                            		}
                            		if (options & OPTIONS_TEMP)
                            		{
                            			...
                            			{
											Rprintf("ERROR: sqlite bind double (temperature).\n");
										}
                            			j++;
                            		}
                            		if (options & OPTIONS_BATT)
                            		{
                            			...
                            			{
											Rprintf("ERROR: sqlite bind double (battery).\n");
										}
                            		}
                                }
                                else
                                ...
                                if (format == FORMAT_SQLITE)
                                {
                                    ...
                                    {
                                        Rprintf("ERROR: Could not step (execute) SQL stmt: %s\n", sqlite3_errmsg(dbconn));
                                        return 1;
                                    }
                                    sqlite3_reset(stmt);
                                }
...
atexit(_getch);
    ...
    for (i = 1; i < argc; i++)
    {
        if (argv[i][0] == '-' || argv[i][0] == '/')
        {
            ...
            {
                Rprintf("ERROR: Unrecognized parameter: %s\n", argv[i]);
                help = 1;
            }
        }
        else
        {
            ...
            {
                Rprintf("ERROR: Too many positional parameters (%d): %s\n", positional + 1, argv[i]);
                help = 1;
            }
            positional++;
        }
    ...
    if (filename == NULL)
    {
        Rprintf("ERROR: File not specified\n");
        help = 1;
    }

    if (help)
    {
        Rprintf("CWA-Convert by Daniel Jackson, 2010-2012\n");
        Rprintf("Usage: CWA <filename.cwa> [-s:accel|-s:gyro] [-f:csv|-f:raw|-f:wav"
#ifdef SQLITE
            "|-f:sqlite"
#endif
...
#ifdef _DEBUG
    Rprintf("DEBUG: Opening file: %s\n", filename);
#endif

    if (DumpFile(filename, outfilename, stream, format, values, time, options, amplify, iStart, iLength, iStep, blockStart, blockCount, verbose))
	{
	    Rprintf("ERROR: Problem dumping file: %s (check exists, readable and not corrupted.)\n", filename);
	}
/*
    cwa = CwaRead(filename);
    if (cwa == NULL)
    {
        Rprintf("ERROR: Problem reading file: %s\n", filename);
        return 4;
    }
